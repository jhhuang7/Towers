...
from enemy import AbstractEnemy, SimpleEnemy, InvincibleEnemy, AdvancedEnemy
...
#2.2 Implement a subclass of AbstractTower that deals damage for use
    #against your custom enemy.
class Energy(AbstractObstacle):
    """A energy beam fired from a EnergyTower that damages all the custom
    enemies it collides with"""
    name = "Energy"
    ...
    def __init__(self, position, cell_size, direction, size = 0.04,
                 rotation: Union[int, float] = 0, grid_speed = 0.15,
                 damage = 30, hits = 30):
        super().__init__(position, (size, 0), cell_size,
                         grid_speed = grid_speed,
                         rotation = rotation, damage = damage)
    ...
    def step(self, units):
        """Performs a time step for this energy

        Moves according to direction, damaging any enemies that are collided with along the way
        If hits is non-zero, this energy expires if it has the number of enemies hit is at least 'hits',
...
class EnergyTower(AbstractTower):
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9),
                 rotation=math.pi * .25, base_damage=90, level: int = 1):
...
class Fire(AbstractObstacle):
    ...
    def __init__(self, position, cell_size, target: AbstractEnemy, size=.5,
                 rotation: Union[int, float] = 0, grid_speed=.5, damage=100):
        super().__init__(position, (size, 0), cell_size, grid_speed=grid_speed, rotation=rotation, damage=damage)
        self.target = target
...
class FireTower(AbstractTower):
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9),
                 rotation=math.pi * .25, base_damage=150, level: int = 1):
        super().__init__(cell_size, grid_size=grid_size, rotation=rotation,
                         base_damage=base_damage, level=level)
    ...
    def step(self, units):
        ...
        # Spawn missile on tower
        fire = Fire(self.position, self.cell_size, target,
                    rotation=self.rotation,
                          damage=self.get_damage(), grid_speed=.1)
