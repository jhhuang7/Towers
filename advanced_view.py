...
from tower import AbstractTower, MissileTower, EnergyTower, SimpleTower, \
    AbstractObstacle, Missile, Energy, Fire, FireTower
...
class TowerView(SimpleView):
    ...
    @classmethod
    def _draw_energy(cls, canvas: tk.Canvas, tower_: SimpleTower):
...
class ObstacleView(SimpleView):
    ...
    @classmethod
    def _draw_fire(cls, canvas: tk.Canvas, fire: Fire):
        """Draws a fire"""

        x, y = fire.position

        length, width = fire.size

        dx, dy = rotate_point((length / 2, width / 2), fire.rotation)
